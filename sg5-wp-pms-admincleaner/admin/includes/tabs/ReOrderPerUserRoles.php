<div id="mopr_settings" class="tabcontent" style="display: none;">
    <div class="DP_AC_admin_card">
        <div class="DP_AC_admin_header">
            <h2>Reorder menu items per user role</h2>
        </div>
        <?php
            $super_admin = get_user_by('ID', $this->DP_AC_super_admin);
            $DP_AC_reorder_force = get_option('DP_AC_reorder_force');
        ?>
        <?php if( $status !== false && $status == 'valid' ){ ?>
            <div class="DP_AC_admin_body">
                <div class="panel" id="panel-mopr_settings-tools">
                    <div class="panel-body clearfix">
                        <div class="dp__switch">    
                            <label>
                                <input type="checkbox" id="DP_AC_reorder_force" <?= (($DP_AC_reorder_force == "yes") ? 'checked' : '') ?> />
                                <div class="rwmb-switch-status">
                                    <span class="rwmb-switch-slider"></span>
                                    <span class="rwmb-switch-on">On</span>
                                    <span class="rwmb-switch-off"></span>
                                </div>
                                <div class="role-name">Force menu order</div>
                            </label>
                        </div>
                    </div>
                </div>
            </div>
            <div class="DP_AC_admin_header">
                <h2>Select role:</h2>
                <select class="select-option" id="mopr_settings_role">
                <option value="">---Select role---</option>
                <?php
                global $wp_roles;
                $all_roles = $wp_roles->roles;
        
                $html = '';
                foreach($all_roles as $role_slug => $role){
                    if($role_slug == "administrator" && $current_user->ID != $this->DP_AC_super_admin){ continue; }
                    $html .= '<option value="'.$role_slug.'">'.$role['name'].'</option>';
                }
                echo $html;
                ?>
                </select>
                <?php if($current_user->ID == $this->DP_AC_super_admin){ ?>
                    <p><b>Administrator option</b> will apply to administrators that aren't super admins (<?php echo $super_admin->data->user_login; ?>). </p>
                <?php } ?>
            </div>
        <?php } ?>
        <div class="DP_AC_admin_body" id="order-per-role-body" style="padding:0px;">
            <div class="panel" id="panel-mopr_settings-tools">
                <div class="panel-body clearfix">
                    <?php echo $mopr_settings_message; ?>
                </div>
            </div>
        </div>
    </div>
</div>